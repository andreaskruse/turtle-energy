...
use turtle_energy::{Error, Result};

fn main() -> Result<()> {
    let cl_input = parse_cl_args()?;

    let res = flow(&cl_input.position_file, &cl_input.price_file);
...
fn flow(position_file: &str, price_file: &str) -> Result<()> {
    let now = Instant::now();
    let deliveries = NaivePositions::from_csv(position_file)?;
    println!("Loading positions took: {:?}", now.elapsed());

    let now = Instant::now();
    let curve = Curve::from_csv(price_file)?;
    ...
    match args.next().as_deref() {
        ...
        None => {
            return Err(Error::Custom(String::from(
                "Need a path to a .csv file with positions: -f <file>",
            )));
        }
    }
